...
from chia.protocols.wallet_protocol import RespondToCoinUpdates, CoinState
...
class SimpleWalletNode:
    ...
    async def on_connect(self, peer: WSChiaConnection):
        ...
        await self.get_coin_state(peer)

    async def get_coin_state(self, full_node: WSChiaConnection):
        all_puzzle_hashes = list(await self.wallet_state_manager.puzzle_store.get_all_puzzle_hashes())
        msg = wallet_protocol.RegisterForPhUpdates(all_puzzle_hashes, 0)
        breakpoint()
        all_state: Union[Optional, RespondToCoinUpdates] = await full_node.register_interest_in_puzzle_hash(msg)
        breakpoint()

    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate):
        # TODO save coin state
        pass

    async def handle_coin_state_change(self, state_updates: List[CoinState]):
        # TODO implement this
        # for coin_state in state_updates:
        pass
